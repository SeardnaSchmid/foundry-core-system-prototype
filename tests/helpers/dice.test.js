...
describe('Edgefall Dice System', () => {
  describe('dieCountFor', () => {
    it('should return 2 dice for simple advantage', () => {
      expect(dieCountFor(EDGEFALL_ADVANTAGE.advantage)).toBe(2);
    });

    it('should return 2 dice for simple disadvantage', () => {
      expect(dieCountFor(EDGEFALL_ADVANTAGE.disadvantage)).toBe(2);
    });

    it('should return 3 dice for none', () => {
      expect(dieCountFor(EDGEFALL_ADVANTAGE.none)).toBe(3);
    });

    it('should return 3 dice for strong advantage', () => {
      expect(dieCountFor(EDGEFALL_ADVANTAGE.strongAdvantage)).toBe(3);
    });

    it('should return 3 dice for strong disadvantage', () => {
      expect(dieCountFor(EDGEFALL_ADVANTAGE.strongDisadvantage)).toBe(3);
    });
  });

  describe('pickCountingDie - Standard Roll (None)', () => {
    it('C1: should pick middle die from [12, 19, 8]', () => {
      const result = pickCountingDie([12, 19, 8], EDGEFALL_ADVANTAGE.none);
      expect(result.value).toBe(12);
    });

    it('should pick middle die from sorted values', () => {
      const result = pickCountingDie([5, 15, 10], EDGEFALL_ADVANTAGE.none);
      expect(result.value).toBe(10);
    });

    it('should handle all same values', () => {
      const result = pickCountingDie([10, 10, 10], EDGEFALL_ADVANTAGE.none);
      expect(result.value).toBe(10);
    });
  });
  ...
  describe('pickCountingDie - Simple Advantage', () => {
    it('C2: should pick lower die from [8, 15]', () => {
      const result = pickCountingDie([8, 15], EDGEFALL_ADVANTAGE.advantage);
      expect(result.value).toBe(8);
    });

    it('should pick lower die when reversed', () => {
      const result = pickCountingDie([15, 8], EDGEFALL_ADVANTAGE.advantage);
      expect(result.value).toBe(8);
    });
  });
  ...
  describe('pickCountingDie - Simple Disadvantage', () => {
    it('C3: should pick higher die from [8, 15]', () => {
      const result = pickCountingDie([8, 15], EDGEFALL_ADVANTAGE.disadvantage);
      expect(result.value).toBe(15);
    });

    it('should pick higher die when reversed', () => {
      const result = pickCountingDie([15, 8], EDGEFALL_ADVANTAGE.disadvantage);
      expect(result.value).toBe(15);
    });
  });
  ...
  describe('pickCountingDie - Strong Advantage', () => {
    it('C4: should pick lowest die from [8, 15, 12]', () => {
      const result = pickCountingDie([8, 15, 12], EDGEFALL_ADVANTAGE.strongAdvantage);
      expect(result.value).toBe(8);
    });

    it('should pick lowest die from any order', () => {
      const result = pickCountingDie([19, 5, 12], EDGEFALL_ADVANTAGE.strongAdvantage);
  ...
  describe('pickCountingDie - Strong Disadvantage', () => {
    it('C5: should pick highest die from [8, 15, 12]', () => {
      const result = pickCountingDie([8, 15, 12], EDGEFALL_ADVANTAGE.strongDisadvantage);
      expect(result.value).toBe(15);
    });

    it('should pick highest die from any order', () => {
      const result = pickCountingDie([5, 19, 12], EDGEFALL_ADVANTAGE.strongDisadvantage);
  ...
  describe('criticalResultFor - Standard Roll (None)', () => {
    it('T1: [1, 1, 15] should be critical success', () => {
      expect(criticalResultFor([1, 1, 15], EDGEFALL_ADVANTAGE.none)).toBe('criticalSuccess');
    });

    it('T2: [20, 20, 5] should be critical failure', () => {
      expect(criticalResultFor([20, 20, 5], EDGEFALL_ADVANTAGE.none)).toBe('criticalFailure');
    });

    it('T3: [1, 10, 15] should be null', () => {
      expect(criticalResultFor([1, 10, 15], EDGEFALL_ADVANTAGE.none)).toBeNull();
    });

    it('should detect all ones as critical success', () => {
      expect(criticalResultFor([1, 1, 1], EDGEFALL_ADVANTAGE.none)).toBe('criticalSuccess');
    });

    it('should detect all twenties as critical failure', () => {
      expect(criticalResultFor([20, 20, 20], EDGEFALL_ADVANTAGE.none)).toBe('criticalFailure');
    });
  });

  describe('criticalResultFor - Simple Advantage', () => {
    it('T4: [1, 15] should be critical success', () => {
      expect(criticalResultFor([1, 15], EDGEFALL_ADVANTAGE.advantage)).toBe('criticalSuccess');
    });

    it('T5: [20, 20] should be critical failure', () => {
      expect(criticalResultFor([20, 20], EDGEFALL_ADVANTAGE.advantage)).toBe('criticalFailure');
    });

    it('T6: [20, 10] should be null', () => {
      expect(criticalResultFor([20, 10], EDGEFALL_ADVANTAGE.advantage)).toBeNull();
    });

    it('should trigger on single one', () => {
      expect(criticalResultFor([1, 20], EDGEFALL_ADVANTAGE.advantage)).toBe('criticalSuccess');
    });

    it('should not trigger critical failure on single twenty', () => {
      expect(criticalResultFor([20, 5], EDGEFALL_ADVANTAGE.advantage)).toBeNull();
    });
  });

  describe('criticalResultFor - Simple Disadvantage', () => {
    it('T7: [20, 10] should be critical failure', () => {
      expect(criticalResultFor([20, 10], EDGEFALL_ADVANTAGE.disadvantage)).toBe('criticalFailure');
    });

    it('T8: [1, 1] should be critical success', () => {
      expect(criticalResultFor([1, 1], EDGEFALL_ADVANTAGE.disadvantage)).toBe('criticalSuccess');
    });

    it('T9: [1, 10] should be null', () => {
      expect(criticalResultFor([1, 10], EDGEFALL_ADVANTAGE.disadvantage)).toBeNull();
    });

    it('should trigger on single twenty', () => {
      expect(criticalResultFor([20, 1], EDGEFALL_ADVANTAGE.disadvantage)).toBe('criticalFailure');
    });

    it('should not trigger critical success on single one', () => {
      expect(criticalResultFor([1, 15], EDGEFALL_ADVANTAGE.disadvantage)).toBeNull();
    });
  });

  describe('criticalResultFor - Strong Advantage', () => {
    it('T10: [1, 15, 10] should be critical success', () => {
      expect(criticalResultFor([1, 15, 10], EDGEFALL_ADVANTAGE.strongAdvantage)).toBe('criticalSuccess');
    });

    it('T11: [20, 20, 20] should be critical failure', () => {
      expect(criticalResultFor([20, 20, 20], EDGEFALL_ADVANTAGE.strongAdvantage)).toBe('criticalFailure');
    });

    it('T12: [20, 20, 10] should be null', () => {
      expect(criticalResultFor([20, 20, 10], EDGEFALL_ADVANTAGE.strongAdvantage)).toBeNull();
    });

    it('should trigger critical success on single one', () => {
      expect(criticalResultFor([1, 19, 18], EDGEFALL_ADVANTAGE.strongAdvantage)).toBe('criticalSuccess');
    });

    it('should not trigger critical failure on two twenties', () => {
      expect(criticalResultFor([20, 20, 15], EDGEFALL_ADVANTAGE.strongAdvantage)).toBeNull();
    });
  });

  describe('criticalResultFor - Strong Disadvantage', () => {
    it('T13: [20, 10, 5] should be critical failure', () => {
      expect(criticalResultFor([20, 10, 5], EDGEFALL_ADVANTAGE.strongDisadvantage)).toBe('criticalFailure');
    });

    it('T14: [1, 1, 1] should be critical success', () => {
      expect(criticalResultFor([1, 1, 1], EDGEFALL_ADVANTAGE.strongDisadvantage)).toBe('criticalSuccess');
    });

    it('T15: [1, 1, 10] should be null', () => {
      expect(criticalResultFor([1, 1, 10], EDGEFALL_ADVANTAGE.strongDisadvantage)).toBeNull();
    });

    it('should trigger critical failure on single twenty', () => {
      expect(criticalResultFor([20, 5, 3], EDGEFALL_ADVANTAGE.strongDisadvantage)).toBe('criticalFailure');
    });

    it('should not trigger critical success on two ones', () => {
      expect(criticalResultFor([1, 1, 15], EDGEFALL_ADVANTAGE.strongDisadvantage)).toBeNull();
    });
  });
  ...
  describe('Integration Tests - Complete Roll Flow', () => {
    it('should succeed with middle die 12 against threshold 14', () => {
      ...
      const threshold = 14;
      const advantage = EDGEFALL_ADVANTAGE.none;
    ...
    it('should succeed with simple advantage lower die', () => {
      const values = [8, 15];
      const threshold = 10;
      const advantage = EDGEFALL_ADVANTAGE.advantage;
    ...
    it('should fail with simple disadvantage higher die', () => {
      const values = [8, 15];
      const threshold = 10;
      const advantage = EDGEFALL_ADVANTAGE.disadvantage;
  ...
  describe('Edge Cases', () => {
    it('should handle all dice showing same value', () => {
      ...
      const counting = pickCountingDie(values, EDGEFALL_ADVANTAGE.none);
    ...
    it('should preserve die index in counting die result', () => {
      ...
      const result = pickCountingDie(values, EDGEFALL_ADVANTAGE.none);
      expect(result.value).toBe(12);
