...
| `maneuvers.mjs` | `ansageKosten`, `MANEUVERS`, `BETRAG_MODES`, `maneuverSkill`, `declarableManeuvers`, `priceAnsagen`, `ansageEnvelope`, `DAMAGE_RULES`, `damageDistribution`, `DEFAULT_ZONE` | The Ansage arithmetic and the nine declarable Manöver. Pure, no Foundry globals; `label` and `effect` are i18n keys — see [combat-roll-workflows.md](../concepts/combat-roll-workflows.md) |
| `combat-actions.mjs` | `angriffOptions`, `paradeOptions`, `ausweichenOptions`, `widerstandOptions`, `actorStance`, `canDefend`, `defenseMalus`, `countDefense`, `takeStance`, `takeAnsage`, `clearAnsage`, `REPEATED_DEFENSE_MALUS` | One builder per Handlung, plus the Haltung rules. Every combat roll is assembled here and nowhere else, so an Ansage can reach the same options an ordinary attack builds |
