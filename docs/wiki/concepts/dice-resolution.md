...
resource: [module/helpers/dice.mjs, module/helpers/dice-odds.mjs]
...
[`module/helpers/dice.mjs`](../../../module/helpers/dice.mjs). Its only import
is the odds read side below; nothing imports back into it, so it stays the base
of the roll graph (see [layering.md](../architecture/layering.md)).
...
The chance a given (threshold, advantage) pair succeeds is a constant, so it is
not computed at runtime. `scripts/dice-odds.mjs` enumerates the entire outcome
space (8,000 hands for 3d20, 400 for 2d20) *through the three functions above*
and writes two artifacts, both regenerated by `npm run docs:odds`:

- [`module/helpers/dice-odds-table.mjs`](../../../module/helpers/dice-odds-table.mjs)
  — generated, never hand-edited. The shipped lookup table.
- [`docs/design/dice-odds.md`](../../design/dice-odds.md) — the readable
  version, plus a Monte Carlo cross-check of the enumeration.

[`module/helpers/dice-odds.mjs`](../../../module/helpers/dice-odds.mjs) is the
read side: `successChanceFor()` (a clamped table lookup), `formatChance()`, and
`oddsTooltipHtml()`, which builds the `data-tooltip-html` card shown on the roll
dialog's threshold box and on a chat card's threshold chip. It imports nothing
from `dice.mjs` — `dice.mjs` calls *it* to put the odds on a roll card — so each
table entry carries its own advantage `key` and die count instead.

**Regenerate after any change to `pickCountingDie` or `criticalResultFor`.**
`tests/helpers/dice-odds.test.js` re-derives the whole table from the live
helpers and fails if the committed one has gone stale, so a forgotten
regeneration surfaces as a red test rather than as wrong odds in play.
