...
- **Manual adjustment:** The edge value is directly editable on the sheet (clamped 0..max). Every manual edit is announced in chat (`Chat.EdgeAdjusted`), in both directions — the message names the edit as manual and shows both ends of it (`from`/`to` out of `max`), so it reads as bookkeeping rather than as a spend or a refund the mechanics granted. A no-op edit (clamped back onto the current value) announces nothing.
...
- **No chat announcements, no extra roll cards (v2.2):** every edge action writes only to `flags.tno.edge` on the triggering message and renders on that card. `Post-mortem` and `Retry` roll their dice in place via `rollInPlace(advantage, threshold)` in [dice.mjs](../../module/helpers/dice.mjs) — the same technique `rerollTrialError` uses (evaluate a `Roll`, fire `game.dice3d.showForRoll` for the roller or fall back to the dice sound, never `ChatMessage.create`). The only edge chat message left is `Chat.EdgeAdjusted` (a manual sheet pool edit, which has no roll to attach to).
...
- `TNO.Chat.EdgeAdjusted` — the only remaining edge chat message, for a manual pool edit on the sheet, formatted with `{name}`/`{from}`/`{to}`/`{max}`.
