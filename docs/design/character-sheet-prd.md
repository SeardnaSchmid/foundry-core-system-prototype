...
Renders `templates/actor/parts/actor-items.hbs`, populated by `_prepareItems()`: owned items are bucketed into `gear` (type `item`), `armory` (type `armor`), `weapons` (type `weapon`), `features` (type `feature`), and `spells` (type `spell`, sub-bucketed 0–9 by `system.spellLevel`). The flat list itself renders `inventory` — the three physical buckets merged back in `sort` order — because everything the inventory rules touch has to be reachable there: a piece of armour that is neither worn nor carried appears in no other view, so leaving it out would strand it. Standard Foundry item-sheet affordances apply (edit/delete, drag-to-macro for owners). One create control sits in the list header, opening the same add dialog as the carry grid's `+`: with three physical types, the type is a field inside the act of adding something rather than a control per type.
...
The two visual views used to sit above that flat list and now live in the Basics tab's right-hand column, above the reserved Waffen block — they answer "what am I wearing / hauling right now?", which is asked mid-roll rather than while bookkeeping. Both are character-only (the NPC sheet shares the items partial but has neither an equipment store nor a slot budget) and derived on every render from `_prepareEquipment()` — nothing about either arrangement is persisted:

- **Paper doll** (`parts/actor-paperdoll.hbs`) — the Unterkleidung as a base-layer row above the four hit locations (Kopf, Torso, Arme, Beine), each with its effective RH/RW/RA. The silhouette paints three states per zone, decided in the sheet as `z.state`: `bare` (grey — nothing here), `suited` (pale green — covered by the Unterkleidung alone, which closes coverage but grants no hardness) and `filled` (green — a zone addon is worn). **An empty zone is a drop target and nothing else:** a piece is worn by dragging it out of the carry grid onto the zone it was authored for, and while one is in flight that zone lights up. Clicking an empty zone used to offer to author a piece on the spot, which conjured armour out of an empty doll — wearing something is a state change on gear already in hand, so the click path is gone. Taking a piece off is still the row's `x`, since there is nothing to drag it from.
- **Trageslots** (`parts/actor-slot-grid.hbs`) — carried gear packed into the slot budget in its existing `sort` order, one grid column per slot. Neither worn clothing and armour (exempt from the Inventarregeln) nor stowed gear (not on the character at all) ever appear; zero-slot items (Geld, Papiere, Krimskrams) drop into a labelled band under the grid rather than zero-width cells — enclosed, so it stays part of the same bag, and half a filled cell tall, so the height says "less than one slot" before the label is read — though armour never can — it is floored at one slot per piece, so a piece that is not worn always takes a visible cell instead of riding along free among the loose change. Cells are draggable: onto another cell to re-sort the flat list, onto a free cell to move the item to the end, onto a paper doll zone to wear it; a click opens the item's own sheet. The zero-slot band behaves identically — costing no slot does not make a thing a different kind of thing. The header carries the used/capacity read-out, the `Keine Tasche` badge (the only state left here: not a movement consequence but the reason the budget reads 0) and a `+` opening the add dialog. **The grid holds exactly as many cells as the character has slots** — no padding out to the raster width, so a capacity of 6 in the five-wide grid just leaves a short second row. Gear that does not fit runs on past the budget in the solid warning colour, with a note counting the excess: the run of normal cells *is* the capacity, and the colour break is what marks where it ended. A block only stays inside the budget if it fits there whole — an item straddling the boundary has overflowed — and once one overflows every later item follows it out, so the grid never reorders the player's list behind their back.

- **Waffen** (`parts/actor-weapons.hbs`) — column headers and a "not yet implemented" marker, no data behind it. The `weapon` item type exists (`quantity`, `slots`, and free-text `dice`/`damage`/`range`) so a weapon can be owned, carried and priced in slots like the object it is, but **how a weapon is readied is still an open rules question**, and a carried item is not a readied weapon. The block holds its place in the column's proportions rather than being left out; do not fill it from `inventory`.
...
- `TNO.Inventory.*` — Trageslots view: title, slot-cost hints, the `Keine Tasche` badge, the add-dialog's labels, the cell/trinket/free-cell hints. The two load-state hints moved to `TNO.DerivedHint.NoSprint` / `CrawlOnly`, where the movement chip reads them.
- `TNO.Armor.*` — paper doll: zone labels (`TNO.Armor.Zone.*`), the RH/RW/RA long/short/hint triples, the unequip action, the drop hint on an empty zone, the wrong-zone warning, and the Stärkevorraussetzung warning.
- `TNO.Weapons.*` — the reserved Waffen block's column captions and "not yet implemented" marker, plus the weapon item sheet's value labels.
