import { TNO_ADVANTAGE, TNO_ADVANTAGE_ABBR, rollTno } from '../helpers/dice.mjs';
...
export class TnoRollDialog extends FormApplication {
  ...
  /** @override */
  get title() {
    return game.i18n.localize('TNO.Roll.DialogTitle');
  }
  ...
  activateListeners(html) {
    ...
    html.on('change', 'select[name="attributeA"], select[name="attributeB"]', (ev) => {
      ...
      html.find('.tno-threshold-value').text(this._computeThreshold(data));
    });
    ...
    html.on('click', '.tno-attribute-chip', (ev) => {
      ...
      const group = $(chip).closest('.tno-attribute-chip-grid');
      group.find('.tno-attribute-chip').removeClass('active');
    ...
    html.on('click', '.tno-bonus-stepper', (ev) => {
      ...
      input.value = next;
      form.querySelector('.tno-bonus-value').textContent = next;
      const data = new FormDataExtended(form).object;
      data.bonus = next;
      form.querySelector('.tno-threshold-value').textContent = this._computeThreshold(data);
    });
  ...
  async _updateObject(event, formData) {
    ...
    if (this.fixedValue) {
      ...
      if (this.skill) {
        ...
        components.push({ label: game.i18n.localize('TNO.Roll.FreeSkillValue'), value: this._freeSkillValue(formData) });
