 ...
 * built-in skill from CONFIG.JOSTER.skills (label pre-localized), plus any
 ...
 * definitions through this helper rather than CONFIG.JOSTER.skills
...
export function getSkillDefinitions(actor) {
  ...
  for (const [key, skill] of Object.entries(CONFIG.JOSTER.skills)) {
    definitions[key] = {
  ...
  for (const [key, entry] of Object.entries(skills)) {
    ...
    definitions[key] = {
      ...
      category: custom.category in CONFIG.JOSTER.skillCategories ? custom.category : DEFAULT_CATEGORY,
      attribute: custom.attribute in CONFIG.JOSTER.abilities ? custom.attribute : DEFAULT_ATTRIBUTE,
      starter: false,
      custom: true,
    };
  }
